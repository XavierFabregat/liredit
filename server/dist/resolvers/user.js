...
let UserResolver = exports.UserResolver = class UserResolver {
    ...
    async changePassword(token, newPassword, { redis, req }) {
        ...
        const user = await entities_1.User.findOne({ where: { id: userIdNum } });
        if (!user) {
            return {
                errors: [
                    {
        ...
        await entities_1.User.update({ id: userIdNum }, { password: newHashedPassword });
        await redis.del(constants_1.FORGET_PASSWORD_PREFIX + token);
        req.session.userId = user.id;
        return { user };
    }
    async forgotPassword(email, { redis }) {
        const user = await entities_1.User.findOne({ where: { email } });
        if (!user) {
            return true;
        }
    ...
    async me({ req }) {
        ...
        return await entities_1.User.findOne({ where: { id: req.session.userId } });
    }
    async register(options, { req }) {
        ...
        let user;
        try {
            user = await entities_1.User.create({
